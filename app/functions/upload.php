        ...
        if(exif_imagetype($_FILES["fileUpload"]["tmp_name"])){ // Check if the file is actually an image.
            if(isset($_POST['profile'])){
                ...
                if(move_uploaded_file($_FILES["fileUpload"]["tmp_name"], $filepath)){
                    $sql5 = "INSERT INTO posts (post_caption, post_public, post_time, post_by)     -- posting at the time of changing dp -- 
                             VALUES ('" . $row['user_firstname'] . " " . $row['user_lastname'] . " has changed his/her profile picture.', 'N', NOW(), {$_SESSION['user_id']})";
